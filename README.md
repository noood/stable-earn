...
产品目录迁移使用 `drizzle/0004_product_catalog.sql`：目录按账号保存内部产品 ID、平台身份和归档状态；新用户不会预置所有平台，公开 API 或账户 API 成功发现产品后才加入目录，人工产品由用户主动添加。API 身份变化会生成新行，旧行只归档不覆盖。已有环境升级时先执行该 migration，再部署代码。
