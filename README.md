...
页脚右侧的 GitHub 标识指向本项目的[公开源码仓库](https://github.com/noood/stable-earn)。

## 数据来源

产品信息与持仓分别标记为 API 或人工维护：API 字段保持只读，未自动获取的数据可以人工维护。私人页面每天在上海时间 08:00 和 20:00 更新，也支持手动刷新。计划更新失败时会在 1 分钟和 5 分钟后重试，前两次失败不会改变用户看到的数据。
...
产品目录、筛选、身份和归档规则见 [数据状态说明](docs/DATA-STATES.md)。已有环境升级时按文件顺序执行尚未运行的 migrations，再部署代码；兼容迁移仍需保留，不应重复修改或删除。
